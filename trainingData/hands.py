def determine_hand(hand):
    ...
    #duplicat_cards = {x for x in cards if cards.count(x) > 1}
    print(same_cards)
...
<<<<<<< Updated upstream:trainingData/hands.py
...
=======
        ...
        if max(same_cards) == 3: # Three of a kind
            ...
            return 3
    elif len(same_cards) == 6:
        return 2
    else: # Garbage hand most likely. But maybe a straight!
        if is_straight(card_nums):
            return 5
        return 1
>>>>>>> Stashed changes:PokerGame/packages/hands.py
